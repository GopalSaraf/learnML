...
class ZScoreNormalization(IFeatureScaling):
    ...
    def fit_transform(self, data: np.ndarray = None) -> np.ndarray:
        ...
        if self._indexes is None:
            return (self._data - self._get_mean()) / self._get_std()
        else:
            normalized_data = np.zeros(self._data.shape)
            normalized_data[:, self._indexes] = (
                self._data[:, self._indexes] - self._get_mean()[self._indexes]
            ) / self._get_std()[self._indexes]
            normalized_data[
                :, [i for i in range(self._data.shape[1]) if i not in self._indexes]
            ] = self._data[
                :, [i for i in range(self._data.shape[1]) if i not in self._indexes]
            ]
